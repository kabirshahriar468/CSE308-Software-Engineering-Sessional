    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (command.equals("Create")) {
                ...
                if (ac_type.equals("Student")) ac = new StudentAcc();
                else if (ac_type.equals("Savings")) ac = new SavingsAcc();
                else if (ac_type.equals("FixedDeposit")) ac = new FixedDepositAcc();

                if (ac.createAccount(init_balance, name)) {
                    b.addNewAccount(ac);
                    System.out.println(ac_type + " account for " + ac.getAccountName() + " Created; initial balance " + ac.getTotalDeposit() + "$");
            ...
            if (command.equals("Open")) {
                ...
                } else if (b.isAccountHolder(name)) {
                    curAccount = (Account) b.getPeople(name);
                    isEmployeeLoggedIn = false;

                    System.out.println("Welcome back," + curAccount.getAccountName());
                } else {
                    System.out.println("Your entered name does not exist.");
                }
            }
            if (command.equals("Deposit")) {
                ...
                if (!isEmployeeLoggedIn) {
                    if (curAccount.deposit(amount)) {
                        System.out.println(amount + "$ deposited; current balance " + curAccount.getTotalDeposit() + "$");
                    } else {
            ...
            if (command.equals("Withdraw")) {
                ...
                if (!isEmployeeLoggedIn) {
                    if (curAccount.withdraw(amount)) {
                        System.out.println(amount + "$ Withdrawn ; current balance " + curAccount.getTotalDeposit() + "$");
                    } else {
                        System.out.println("Invalid Transaction; current balance " + curAccount.getTotalDeposit() + "$");
                    }
                } else if (isEmployeeLoggedIn) {
                    System.out.println("You are in employee account , login from customer account");
                }
            ...
            if (command.equals("Query")) {
                if (!isEmployeeLoggedIn) {
                    System.out.println("Current balance " + curAccount.getTotalDeposit() + "$ , Loan " + curAccount.getLoan() + "$");
                } else if (isEmployeeLoggedIn) {
                    System.out.println("You are in employee account , login from customer account");
                }
            }
            ...
            if (command.equals("Close")) {
                if (!isEmployeeLoggedIn) {
                    System.out.println("Transaction Closed for " + curAccount.getAccountName());
                } else {
                    System.out.println("Operations for " + curEmployee.getEmployeeName() + " closed");
                }
